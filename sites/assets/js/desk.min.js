	...
	frappe.ui.form.MultiSelectDialog = Class.extend({
		...
		bind_events: function() {
			...
			this.$parent.on('click', '.btn[data-fieldname="make_new"]', function (e) {
				...
				Object.keys(this$1.setters).forEach(function(setter) {
					frappe.route_options[setter] = me.dialog.fields_dict[setter].get_value() || undefined;
				});
				frappe.new_doc(this$1.doctype, true);
			});
		},
	...
	$.extend(frappe.model, {
		...
		get_default_value: function(df, doc, parent_doc) {
			...
			function is_doc_allowed(doctype, docname) {
				return user_permissions[doctype].some(function (perm) {
		...
		get_match_rules: function (doctype, ptype) {
			...
			if (user_permissions && !$.isEmptyObject(user_permissions)) {
				...
				$.each(fields_to_check, function (i, df) {
					...
					var has_user_permission = user_permissions_for_doctype ? user_permissions_for_doctype
						.some(function (perm) { return !perm.applicable_for || perm.applicable_for === doctype; }) : false;

					if (has_user_permission) {
						rules[df.label] = [];
						user_permissions_for_doctype.map(function (permission) {
	...
	Object.assign(frappe.utils, {
		...
		report_total_accumulator: function(column, values, type) {
			if (column.fieldtype == "Percent" || type === "mean") {
				return values.reduce(function (a, b) { return ({content: a.content + flt(b.content)}); }).content / values.length;
			} else if (frappe.model.is_numeric_field(column.fieldtype)) {
				return values.reduce(function (a, b) { return ({content: a.content + flt(b.content)}); }).content;
			} else {
				return false;
			}
		}
